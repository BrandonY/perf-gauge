        ...
        for bench_run in self.runs.into_iter() {
            ...
            concurrent_clients.push(match bench_protocol_adapter.as_ref().clone() {
                BenchmarkMode::Http(http_bench_session) => tokio::spawn(async move {
                    let mut http_bench_session = http_bench_session.clone();
                    bench_run
                        .send_load(&mut http_bench_session, metrics_channel)
                        .await
                }),
                BenchmarkMode::Gcs(gcs_bench_session) => tokio::spawn(async move {
                    let mut gcs_bench_session = gcs_bench_session.clone();
                     bench_run
                         .send_load(&mut gcs_bench_session, metrics_channel)
                         .await
                }),
            });
        }
